...
# D. Surgical Patch for install_stack.py
# Fix its safe-install regex and indentation logic to match build_ecosystem.sh
if [ -f "apps/control/install_stack.py" ]; then
	echo "[control] Patching install_stack.py for Python 3.14 stability..."
	env/bin/python -c "
import sys, re
path = 'apps/control/install_stack.py'
with open(path, 'r') as f: content = f.read()

# Fix safe-install regex to be more robust
pattern_regex = r'--site\s+([^\s]+)\s+install-app\s+([^\s]+)'
repl_regex = r'--site\s+([^\s]+)\s+install-app\s+([^\s]+)'
content = content.replace(pattern_regex, repl_regex)

# Improve snippets
pattern_snippet = r'snippet = f\"import frappe; frappe.init\(site='\''\{site\}'\''\); frappe.connect\(\); from frappe.installer import install_app; install_app('\''\{app_name\}'\''\)\"'
repl_snippet = 'snippet = f\"import frappe; frappe.init(site=\'{site}\'); frappe.connect(); from frappe.installer import install_app; install_app(\'{app_name}\')\"'
# (Note: snippet looks fine in original, but we ensure it uses the right quotes)
